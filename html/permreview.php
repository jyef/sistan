...
if(isset($_POST['false'])){
    foreach($_POST['false'] as $k => $v){
        if ($v == 'off') {
            $statement = $db->prepare('UPDATE mytable SET perm=false WHERE id=?');
...
$words = $db->prepare('SELECT * FROM mytable WHERE id BETWEEN ' . $start . ' AND ' . $end . ' AND perm = true ORDER BY RAND()');
$words->execute();
?>
<!DOCTYPE html>
<html lang="en">
